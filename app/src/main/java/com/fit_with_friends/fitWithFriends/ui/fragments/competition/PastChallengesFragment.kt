...
import com.fit_with_friends.fitWithFriends.ui.adapters.challenge.PastChallengesAdapter
import com.fit_with_friends.fitWithFriends.utils.Constants
import kotlinx.android.synthetic.main.fragment_past_challenges.*
...
class PastChallengesFragment : BaseV4Fragment() {
    ...
    private lateinit var pastChallengesAdapter: PastChallengesAdapter
    private var pastChallengesModelList: MutableList<ChallengeModel> = ArrayList()
    ...
    private fun getAllChallengesFromServer() {
        startAnim()
        val input = PageInput()
        input.query.add("challenge_type", 2)
            ...
            object : AsyncResult<PagePagination<ChallengeModel>> {
                override fun success(challengeModelPage: PagePagination<ChallengeModel>) {
                    ...
                    activity!!.runOnUiThread {
                        stopAnim()
                        if (swipeRefresh_past != null) {
                            swipeRefresh_past.isRefreshing = false
                        }
                        if (challengeModelPage.body.data.size != 0) {
                            if (recyclerView_past_challenges != null) {
                                recyclerView_past_challenges.visibility = View.VISIBLE
                            }
                            if (textView_no_past_challenges != null) {
                                textView_no_past_challenges.visibility = View.GONE
                            }
                            pastChallengesModelList.clear()
                            pastChallengesModelList.addAll(challengeModelPage.body.data)
                            pastChallengesAdapter.notifyDataSetChanged()
                        } else {
                            if (recyclerView_past_challenges != null) {
                                recyclerView_past_challenges.visibility = View.GONE
                            }
                            if (textView_no_past_challenges != null) {
                                textView_no_past_challenges.visibility = View.VISIBLE
                            }
                        }
                    }
                }
    ...
    private fun setAdapters() {
        pastChallengesAdapter = PastChallengesAdapter(activity, pastChallengesModelList, asyncResult)
        recyclerView_past_challenges.layoutManager = LinearLayoutManager(activity)
        recyclerView_past_challenges.adapter = pastChallengesAdapter
    }
    ...
    private var asyncResult: AsyncResult<ChallengeModel> = object : AsyncResult<ChallengeModel> {
        override fun success(challengeModel: ChallengeModel) {
            ...
            intent.putExtra(Constants.ACTIVITY_TYPE, "past")
    ...
    private fun listeners() {
        swipeRefresh_past.setOnRefreshListener {
            swipeRefresh_past.setColorSchemeResources(
    ...
    private fun startAnim() {
        if (avi_loader_past != null) {
            avi_loader_past.smoothToShow()
        }
    }

    internal fun stopAnim() {
        if (avi_loader_past != null) {
            avi_loader_past.smoothToHide()
        }
    }
}
